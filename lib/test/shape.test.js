const svgSyntax = require("../svgSyntax");
const userInputs = require("../../index");
...
describe("TESTING", () => {
  ...
  describe('Test User Inputs', () => {
      it('Initials Length', async () => {
          const expected = false;
          const actual = await userInputs.isThreeOrLess("NNNN");
          expect(actual).toEqual(expected);
      })
      it('HexDec Syntax', async () => {
          const expected = false;
          const actual = await userInputs.isHex("FFF000");
          expect(actual).toEqual(expected);
      })
  })
});
